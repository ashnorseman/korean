...
const newFile = '../raw-data/yeon-sei-vocabulary-2.txt';
...
const naverResults: RawWordItem[] = [];
...
async function parseWords() {
  const promises = wordList.map(async line => {
    ...
    const result: NaverDictResponse = await getJSON(`https://zh.dict.naver.com/api3/zhko/search?query=${wordName}`).catch((error: any) => console.log(wordName));

    const items = result.searchResultMap?.searchResultListMap?.WORD?.items;
    const hasResult = items?.some(item => item.handleEntry === wordName);

    if (!hasResult) {
      console.log(wordName, items?.map(item => item.handleEntry));
      return;
    }
    ...
    //   //   }
    //   //
    //   //   naverResults.push(new RawWordItem({
